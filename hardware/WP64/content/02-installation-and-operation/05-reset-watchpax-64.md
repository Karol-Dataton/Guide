...
The reset menu is hidden by design in order to avoid accidental resets or misuse. To reset your WATCHPAX 64 device, follow the steps below.

::: carousel

### Power Off the WATCHPAX 64

Shut down the WATCHPAX 64 by pressing the power button on the front panel. Wait until all LED indicators on the unit have turned off completely before proceeding. This ensures the operating system has fully shut down and no files are being written to disk. If the unit is unresponsive, press and hold the power button for approximately five seconds to force a hardware shutdown -- only use this as a last resort.

### Disconnect USB Devices

Remove all USB devices currently connected to the WATCHPAX 64, including mice, flash drives, dongles, and any other peripherals. This is important because external USB devices can interfere with the reset process or cause unexpected behavior during system reconfiguration. Check both the front and rear USB ports to make sure nothing is plugged in.

### Connect a Keyboard

Connect a USB keyboard to one of the USB ports on the WATCHPAX 64. You will need the keyboard to interact with the reset menu during the boot sequence. Any standard wired USB keyboard will work -- there is no need for special drivers or a specific model. A wireless keyboard with a USB receiver is also acceptable. Make sure the keyboard is securely connected before powering on.

### Connect a Display

Connect at least one display device to one of the DisplayPort outputs on the rear panel of the WATCHPAX 64. The display is required so you can see the boot countdown and the reset menu. If you have multiple displays available, connecting just one is sufficient for the reset process. The display must be powered on and set to the correct input source.

### Power On the WATCHPAX 64

Press the power button on the front panel to turn on the WATCHPAX 64. The unit will begin its startup sequence and a five-second countdown will appear in the top-left corner of the connected display. You need to act quickly during this window, so keep your hand on the keyboard and be ready to press **Esc** as soon as the countdown appears.

### Press Esc During the Countdown

As shown below, a five-second countdown appears in the top-left corner of the display. **Press the Esc key on the keyboard before the countdown reaches zero** to enter the reset menu.
...
After the initial reset completes, the WATCHPAX 64 will automatically restart several times in order to reconfigure the operating system, GPU drivers, and hardware settings. This is normal behavior -- each restart configures a different component of the system. The number of restarts may vary, but typically the unit will restart two to three times.
